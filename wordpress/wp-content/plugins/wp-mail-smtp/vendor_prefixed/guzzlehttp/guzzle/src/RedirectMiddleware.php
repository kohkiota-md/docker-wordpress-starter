...
class RedirectMiddleware
{
     ...
     */
    private function withTracking(\WPMailSMTP\Vendor\GuzzleHttp\Promise\PromiseInterface $promise, string $uri, int $statusCode) : \WPMailSMTP\Vendor\GuzzleHttp\Promise\PromiseInterface
    {
        return $promise->then(static function (\WPMailSMTP\Vendor\Psr\Http\Message\ResponseInterface $response) use($uri, $statusCode) {
     ...
     */
    private function guardMax(\WPMailSMTP\Vendor\Psr\Http\Message\RequestInterface $request, \WPMailSMTP\Vendor\Psr\Http\Message\ResponseInterface $response, array &$options) : void
    {
        $current = $options['__redirect_count'] ?? 0;
        $options['__redirect_count'] = $current + 1;
        $max = $options['allow_redirects']['max'];
        if ($options['__redirect_count'] > $max) {
            throw new \WPMailSMTP\Vendor\GuzzleHttp\Exception\TooManyRedirectsException("Will not follow more than {$max} redirects", $request, $response);
        }
    }
    public function modifyRequest(\WPMailSMTP\Vendor\Psr\Http\Message\RequestInterface $request, array $options, \WPMailSMTP\Vendor\Psr\Http\Message\ResponseInterface $response) : \WPMailSMTP\Vendor\Psr\Http\Message\RequestInterface
    {
        ...
        $modify['uri'] = $uri;
        \WPMailSMTP\Vendor\GuzzleHttp\Psr7\Message::rewindBody($request);
        ...
        return \WPMailSMTP\Vendor\GuzzleHttp\Psr7\Utils::modifyRequest($request, $modify);
    }
    /**
     * Set the appropriate URL on the request based on the location header.
     */
    private static function redirectUri(\WPMailSMTP\Vendor\Psr\Http\Message\RequestInterface $request, \WPMailSMTP\Vendor\Psr\Http\Message\ResponseInterface $response, array $protocols) : \WPMailSMTP\Vendor\Psr\Http\Message\UriInterface
    {
